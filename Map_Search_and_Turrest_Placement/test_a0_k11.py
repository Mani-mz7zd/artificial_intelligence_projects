...
def navigate(mapX, moves):
    ...
    you_loc = [(row_i, col_i) for col_i in range(len(mapX[0]))
               for row_i in range(len(mapX)) if mapX[row_i][col_i] == "p"][0]
    # Find goal location
    goal_loc = [(row_i, col_i) for col_i in range(len(mapX[0]))
                for row_i in range(len(mapX)) if mapX[row_i][col_i] == "@"][0]
    moves = moves.lower()
    try:
...
def check_solution1(mapX, dist_key):
    ...
    # function to search
    dist, moves = mystical_castle.search(copy.deepcopy(mapX))
...
def check_solution2(mapX, sol_exist_key, n):
    solved_map, sol_exist = place_turrets.solve(copy.deepcopy(mapX), n)
    import numpy as np
    ...
    if sol_exist_key == True:
        ...
        assert len(set([c for row in solved_map for c in row]) -
                   set(['.', 'X', 'p', '@'])) == 0, "Map contains invalid characters"
        # replacing p on both maps
        solved_map_p = np.array([[c.replace('p', '.')
                                for c in row] for row in solved_map])
        mapX_p = np.array([[c.replace('p', '.') for c in row] for row in mapX])
        assert np.array_equal(
            solved_map_p, mapX_p), "The buildings in the original map replaced"
        p_locs = [(row, col) for col in range(solved_map.shape[1])
                  for row in range(solved_map.shape[0]) if solved_map[row, col] == 'p']
        assert len(p_locs) == n, "Wrong number of pichus placed"
        assert seekthehidden(
            solved_map, p_locs) == True, "Pichus can see each other"
